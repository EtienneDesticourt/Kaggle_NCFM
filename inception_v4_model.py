...
class InceptionV4Model(object):
    ...
    def create_model(self, learning_rate, epochs, batch_size):
        ...
        # Shave off the top
        inception_v4_full = inception_v4_backend.create_model(weights='imagenet')

        for i in range(4):
            inception_v4_full.layers.pop()

        # Add classification for layers for our 8 classes
        output = inception_v4_full.get_layer(index=-1).output # Shape: (8, 8, 1536)
        output = AveragePooling2D((8, 8), strides=(8, 8), name='avg_pool')(output)
        output = Dropout(0.2)(output)
        output = Flatten(name='flatten')(output)
        output = Dense(8, activation='softmax', name='predictions')(output)

        self.model = Model(inception_v4_full.input, output)
